...
`hash_content(obj)` is the function the emitters and `HumanReview` actually call. It first passes `obj` through `_to_serializable`, which recursively unwraps Pydantic and LangChain objects — anything exposing `model_dump` or `dict` — into plain JSON primitives, then delegates to `canonical_json_sha256`. This is what lets the emitters hash a list of `BaseMessage` objects or a raw tool return value without the caller pre-normalising it.

The canonical form is intentionally *close to but not strictly* RFC 8785 (JSON Canonicalisation Scheme). Python's default number formatting diverges from JCS in edge cases such as integer-valued floats. The consequence, stated plainly: an independent verifier must reproduce this implementation's number formatting exactly to recompute a digest, and cannot assume a stock JCS library will agree. Adopting a published JCS implementation is identified as future work in Chapter 6. The divergence is documented in the `canonical_json_sha256` docstring so that it is discoverable from the code, not only from the thesis.
