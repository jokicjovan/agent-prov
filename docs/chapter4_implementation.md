...
- `runtime_metadata` — an optional, non-hashed side field the emitter builds from the runtime identifiers the projection kept out of the content hashes: the framework `run_id`, the provider's response `message_id`, and a map from each canonical tool-call label to the real `tool_call_id` it replaced (§4.5.6). It is omitted when empty.
...
### 4.5.6 Canonical identifier rewriting and `runtime_metadata`

Both content hashes are computed over a *projection* of the LangChain messages, not the raw objects, because LangChain stamps a fresh runtime identifier on every generated message and on every tool call. Hashing those verbatim would make an identical conversation digest differently on each replay, destroying the content-addressable property (§4.5.1). The naive fix is to strip the identifiers; the implementation does something more precise, because some of those identifiers carry meaning.

A tool call's `id` and the `tool_call_id` on the `ToolMessage` that answers it are not noise — together they encode *which result answered which call*. Stripping them discards that edge, and a step that fires several tool calls at once, or receives their results out of order, then becomes indistinguishable from a differently-wired one. So rather than strip these *correlation* identifiers, the projection **rewrites** them. A small `_IdCanonicalizer` assigns deterministic labels — `"0"`, `"1"`, `"2"`, … — in order of first appearance within one projection scope (the input message list, and separately the output generations), and the same runtime id always maps to the same label. A tool call labelled `"0"` and the `ToolMessage` whose `tool_call_id` references it both resolve to `"0"`, so the call-to-result edge is carried *inside* the hashed structure. The label depends only on structure, not on the random runtime value, so the digest stays replay-stable; but because the binding is now explicit rather than positional, two histories with the same calls and results but a different call-to-result pairing digest differently, and a step's parallel tool calls survive intact. The free-standing `AIMessage.id` is treated differently: it correlates with nothing inside a step and is not reliably present across runs, so it is kept out of the digest entirely.

The runtime identifiers the projection removes from the hash are not lost. Each emitter builds a `runtime_metadata` object and passes it to the factory, which attaches it to the record unhashed (and omits it when empty). For an agent step it carries the framework `run_id`, the provider's response `message_id`, and a `tool_call_ids` map from each canonical label back to the real `tool_call_id` it replaced; for a tool invocation it carries the `run_id`. This is a forensic side channel: it lets an investigator cross-reference a record against the provider's request log or the framework's execution trace, which the projected hash alone cannot support. It is deliberately kept out of `input_hash` / `output_hash` — so those stay replay-stable — but it is covered by the Pipeline Bundle's `bundle_hash` seal, so its values are still tamper-evident. Because rewriting changes what the content hashes commit to, it is a breaking change to the protocol, taken as the `0.2.0 → 0.3.0` schema version bump; the schema adds `runtime_metadata` as an optional, open object on the two automated record types.

The tool-calling demonstration in `demos/langchain/tool_calling/` exercises this end to end on a deterministic mock: an agent step whose model emits *two* tool calls in one turn, so its `runtime_metadata.tool_call_ids` records both labels (`{"0": …, "1": …}`), followed by a synthesis step whose input carries the two answering `ToolMessage`s — each rewritten to the label of the call it answers, so the call-to-result correlation is inside that step's `input_hash` and would survive the two results arriving in either order.
...
### 4.7.1 Crash-safe persistence

The session accumulates records *in memory* and the bundle is sealed only after the run finishes (§4.8). Between the first record and the seal, the only copy of the provenance lives in the process. A crash, an out-of-memory kill, or a power loss before sealing therefore discards the entire run's evidence — and a run that fails partway is exactly the case an auditor most wants the record of. The `agent_prov.persistence` package closes this window with an optional durability layer.

`EventLog` (`src/agent_prov/persistence/__init__.py`) is an append-only NDJSON sink. A session is opted in by passing it as `PipelineSession(event_log=...)`; the session writes a header line describing its identity on construction, and `add_record` streams each record to the log as one JSON line *after* updating the in-memory state. Every write is flushed and `fsync`'d before `add_record` returns, so a record the session has accepted is on disk even if the process dies in the next instruction. The log is opened for *exclusive creation*: a log left behind by a crashed run is never silently overwritten by the next one, so the forensic trail is preserved until an operator removes it deliberately. The layer is standard-library only (`json`, `os`, `pathlib`) and lives in the protocol core, requiring no optional extra.

The log is *operational* storage, not part of the hashed evidence. It is plain — not canonical — JSON and carries no integrity hash of its own; the authoritative artefact remains the sealed bundle, whose `bundle_hash` is recomputed from the recovered records and re-validated at seal time. After a crash, `recover_session(path)` replays the log into a fresh `PipelineSession` — reconstructing the session identity from the header and re-appending every logged record in order, restoring `records` and `last_record_id` — which is then sealed exactly as a live session would be. A final line left half-written by a crash is tolerated: an unparseable *last* line is treated as a truncated tail and dropped, while a malformed line anywhere before the end is unrecoverable corruption and raises. A command-line entry point, `python -m agent_prov.persistence <log.ndjson> <bundle.json>`, recovers and seals in one step, defaulting `outcome` to the records-derived value but accepting `--outcome aborted` for a run a crash cut short.

One deliberate divergence from the rest of the implementation is worth noting. The observation layer's standing rule is *never crash the observed pipeline* (§4.4); the event log inverts it. Opting a session into a log is opting into its durability contract, so if a write cannot reach disk the log raises rather than silently dropping the record — a log with a hole is not a recoverable log, and a durability guarantee that fails quietly is worse than none. The in-memory append happens first, so even when the durable write raises, the in-memory bundle the run will seal stays complete.
...
Alongside these four checks the verifier reports one **concurrency observation** as a `warning` rather than an `error`: it groups records whose `timestamp_start`/`timestamp_end` intervals overlap into concurrent clusters and flags each, so an auditor is told where the chronological parent chain (§4.7) orders records sequentially only as an approximation. Overlap is strict — a clean hand-off where one record ends exactly as the next begins is not flagged — and the observation deliberately does *not* affect `ok`, because a parallel pipeline is a valid, untampered bundle rather than a defect. It derives concurrency from observables the records already carry, not from any orchestrator's execution model, so it stays framework-neutral; it detects the false sequential edge without reconstructing the true causal graph, the wider direction developed in §6.5.3.
...
The verifier establishes tamper-*evidence*; an optional layer, `agent_prov.signing`, adds tamper-*proof*. Because `bundle_hash` is an unkeyed digest, anyone who alters a bundle can recompute a fresh, internally consistent seal (§6.2.2). A detached signature over the seal closes that gap: `sign_bundle` produces a small JSON envelope — separate from the bundle, which stays byte-stable — that signs not the bare digest but a *bound* payload, `{payload_type, algorithm, signed_hash}`, canonicalised through the same RFC 8785 path used for the seal, so the signature cannot be lifted onto another artefact. It uses Ed25519, an asymmetric scheme, so `verify_signature` needs only the public key, and it recomputes the seal before checking the signature, so a record edited after signing fails verification even if its `bundle_hash` was honestly re-sealed. The whole layer sits behind the `agent-prov[signing]` extra (it pulls `cryptography`); the protocol core imports nothing cryptographic and verifies tamper-evidence without it. A `python -m agent_prov.signing` CLI exposes `keygen`, `sign`, and `verify`. The design follows established provenance-signing practice (in-toto / SLSA / DSSE / Sigstore); what it deliberately omits — key-identity infrastructure and trusted timestamping — is discussed in §6.5.4.
...
The LangGraph demonstrations live under `demos/langchain/`, grouped to mirror the adapter subpackage they exercise (`agent_prov.adapters.langchain`). The two principal pipelines — the research assistant (§4.11.1) and the document review (§4.11.2) — are the evaluation subjects, and each ships in two variants: a `mock.py` that uses a deterministic fake LLM and makes no network calls, and a `live.py` that uses `ChatOpenAI` against the real API. Two further pipelines are mock-only, each isolating one protocol feature that a scripted deterministic run demonstrates fully: the tool-calling demo (§4.5.6) and the biometric verification demo (§4.11.3). The mock variants are the ones used as running examples throughout: they are deterministic, reproducible in CI, and produce committed bundles that the reader can inspect. The live variants exist to confirm the middleware behaves identically against a real provider and serve as the target for the overhead benchmark in Chapter 5.
...
### 4.11.3 Biometric verification pipeline

A third, mock-only pipeline (`demos/langchain/biometric_review/`) is the concrete artifact for the Article 14(5) two-person rule (§3.7.4). It models a biometric identification system: a `matcher` agent adjudicates the top candidate the matching engine returns for an access-control probe, and — because a biometric identification may not be acted upon until it has been *separately verified and confirmed by at least two natural persons* — a single review gate collects a joint verification from two forensic examiners, producing one `human_intervention` record whose `reviewer_id` carries both. The bundle is sealed with `oversight_regime: "biometric_dual_control"`, which opts the run into the two-person rule, and the validation surface then enforces at seal time that every Human Intervention Record carries at least two distinct reviewers. Had the gate named only one examiner, `BundleGenerator.generate()` would refuse to seal the bundle with a `ProtocolValidationError`, so this demo is the positive artifact for a rule whose negative case (a single reviewer refused) is exercised in the test suite. It is mock-only by design: the two-person verification is a scripted human decision rather than a model call, so a live variant would exercise nothing the mock does not.

---

## 4.12 Testing strategy

The implementation is covered by 230 tests, organised into a unit suite and one integration test.
...
**The parent chain is chronological, not causal.** `parent_record_id` records emission order, not the "issued-by" relationship between an agent step and the tool call it dispatched. For the sequential demo pipelines emission order *is* the causal order, but a pipeline with parallel branches would produce a chain that asserts a sequential edge its records did not obey. The verifier does not accept that assertion blindly: it compares the `timestamp_start`/`timestamp_end` interval of every record and reports any set whose intervals overlap as a concurrency *warning*, so an auditor is told where the chronological ordering is an approximation rather than being silently misled (§6.3). This detects the false edge; it does not yet *reconstruct* the true causal graph, which is left to an analysis layer, to a future optional `caused_by_record_id` field (§3.7.3), or to the content-addressed topology recovery of §6.5.3.

It does not block the evaluation: it is a documented divergence with a defined resolution, and the verifier already flags the case it under-describes. Chapter 5 now turns from how the protocol is implemented to how well it performs: completeness against the Act's obligations, runtime and storage overhead, and instrumentation effort.
